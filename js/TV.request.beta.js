...
*/
const $ = new Env("Apple TV v2.0.0-request-beta");
const URL = new URLs();
const DataBase = {
	"Location":{
	...
	"TV":{
		"Settings":{"Switch":true,"CountryCode":"SG","LangCode":"zh-Hans"}
	},
};
	...
	if (Settings.Switch) {
		...
		switch (url.path) {
			...
			case "uts/v2/favorites/remove":
				$request.body = $request.body.replace(/sf=[\d]{6}/, `sf=${url.params.sf}`);
				break;
			default:
				if (url.params.pfm === "desktop") url.params.pfm = "ipad"
