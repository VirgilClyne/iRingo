...
async function outputNextHour(apiVersion, providerName, minutelyData, weather, Settings) {
	...
	function getSummaries(apiVersion, minutes) {
		...
		// initialize data
        let summary = { condition: (minutes[0].precipIntensity > 0) ? weatherType : "clear" };
		...
		for (let index = 0; index < minutes.length; index++) {
			...
			if (!isRainOrSnow && index + 1 >= minutes.length) {
			...
			if (isRainOrSnow) {
				if (
					...
					if (calculatePL(precipIntensity) === 0) {
					...
					if (apiVersion !== "v1") summary.startTime = startTime;
