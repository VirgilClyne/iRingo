...
*/
const $ = new Env(" iRingo: 🔍 Siri v3.0.0(4) request.beta");
const URL = new URLs();
const DataBase = {
	"Location":{
						...
						default: // 其他主机
							...
							switch (PATH) {
								...
								case "search": // 搜索
									...
									break;
								case "card": // 卡片
									switch (url?.params?.include) {
										case "tv":
										case "movies":
											switch (url?.params?.storefront?.match(/[\d]{6}/g)) { //StoreFront ID, from App Store Region
												case "143463": // CN
													url.params.q = url.params.q.replace(/%2F[a-z]{2}-[A-Z]{2}/, "%2Fzh-HK")
													break;
												case "143470": // TW
													url.params.q = url.params.q.replace(/%2F[a-z]{2}-[A-Z]{2}/, "%2Fzh-TW")
													break;
												case "143464": // SG
													url.params.q = url.params.q.replace(/%2F[a-z]{2}-[A-Z]{2}/, "%2Fzh-SG")
													break;
											};
											break;
										case "apps":
